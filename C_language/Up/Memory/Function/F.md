...
- memcmp 比较内存(用于比较两块内存区域的前n个字节是否相等) --- 以字节为单位比较
    - 函数原型：int memcmp(const void *s1, const void *s2, size_t n);
    - 功能：将 s1 指向的内存区域的前 n 个字节与 s2 指向的内存区域的前 n 个字节进行比较
    - 参数：
        - s1：指向第一个内存区域的指针
        - s2：指向第二个内存区域的指针
        - n：要比较的字节数
    - 返回值：
        - 如果 s1 和 s2 的前 n 个字节相等，则返回 0
        - 如果 s1 的前 n 个字节大于 s2 的前 n 个字节，则返回大于 0 的值
        - 如果 s1 的前 n 个字节小于 s2 的前 n 个字节，则返回小于 0 的值


- memset 填充内存(用于将内存区域的前n个字节设置为指定的值)
    - 函数原型：void *memset(void *s, int c, size_t n);
    - 功能：将 s 指向的内存区域的前 n 个字节设置为指定的值 c
    - 参数：
        - s：指向内存区域的指针
        - c：要设置的值（会被转换为 unsigned char）
        - n：要设置的字节数
    - 返回值：指向内存区域的指针
